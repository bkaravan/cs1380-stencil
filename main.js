...
function main() {
  startNodes(() => {
    const typos = require('typos'); // Add this at the top of the main function
    ...
    const queryService = {
      query: (query) => {
        ...
        // Get typo suggestions for the query
        const suggestions = typos(query);
        const res = [];
        ...
        // If no exact matches, try typo suggestions
        if (res.length === 0 && suggestions.length > 0) {
          for (const line of data) {
            const term = line.split('|')[0];
            for (const suggestion of suggestions) {
              if (term.includes(suggestion)) {
                res.push(line);
        ...
        if (res.length > 0) {
          return {
            results: res,
            originalQuery: query,
            usedSuggestions: res.length > 0 && !res.some(line => line.split('|')[0].includes(query)) 
              ? suggestions 
              : null
          };
        }
        ...
        return {
          ...
          message: suggestions.length > 0 
            ? `No results found for "${query}" or suggestions: ${suggestions.join(', ')}`
    ...
    distribution.mygroup.routes.put(queryService, 'query', (e, v) => {
      ...
      rl.on('line', (line) => {
        ...
        try {
          const remote = {service: 'query', method: 'query'};
          distribution.mygroup.comm.send([line], remote, (e, v) => {
            // Enhanced response formatting
            if (v.results && v.results.length > 0) {
              console.log(`Results for "${v.originalQuery}":`);
              v.results.forEach((result, index) => {
                console.log(`${index + 1}. ${result}`);
              });
              if (v.usedSuggestions) {
                console.log(`(Matched using suggestions: ${v.usedSuggestions.join(', ')})`);
              }
            } else {
